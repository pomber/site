...
Every time a user visits the site, it shows a loading screen until it downloads a big JSON, which is then transformed to less than 10 bytes of information.
...
For that, you can export a `getStaticPaths` function which returns a list of paths. Those paths define the parameters of the route, for the country page that would be the country name, for example, `{ params: { name: "Spain" } }`.
...
Finally, render the country stats using a `Stream` chart. And that's all.
...
Another advantage of pre-rendering every page during build time is that the site may even work without JavaScript.

Since we used a charting library that works on server-side, you can turn off JavaScript and still see the charts and navigate the site. You won't have tooltips, interactions or client-side navigation but the site is still usable.
