...
> ⚠️ This is a draft, I'm still writing this post. Subscribe to the merge event on [this PR](https://github.com/pomber/site/pull/3) to receive a notification when I publish it.
...
You can find the history with the old blog posts and the code on the [Didact repo](https://github.com/pomber/didact). There's also a [talk covering the same content](https://youtu.be/8Kc2REHdwnQ).

But this is a self-contained post. Starting from scratch, these are all the things we'll add to our version of React one by one:
...
_As of August 2019, Concurrent Mode isn't stable in React yet. Currently the loop looks more like this:_
...
If we ignore all the boilerplate needed to iterate over an array and a linked list at the same time, we are left with that matter inside this while: `oldFiber` and `element`. The `element` is the thing we want to render to the dom and the `oldFiber` is what we rendered the last time.
...
_Here React also uses keys, that makes a better reconciliation. For example, it detects when children change places in the elements array._
...
We also add a new property to the fiber: the `effectTag`. We'll use this property later, on the commit phase.
...
You can play with it on [codesandbox](https://codesandbox.io/s/didact-8-21ost).
...
For example, if you add a breakpoint in one of your function components the call stack should show you:
...
- Recycle the objects from old fiber trees
...
- Instead of `requestIdleCallback` React uses its own scheduler
- And many more...
...
If you add any of these or other features to Didact send a pull request to the [GitHub repo](https://github.com/pomber/didact), so others can see it and comment it.

And if you want to comment, like or share this post you can use this tweet:

https://twitter.com/pomber/status/1158024820015190017
