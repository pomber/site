...
We use `TreeMap` from [nivo](https://nivo.rocks/) to display the rows.

The tree map is ugly by default, let's customize it to make it look better.

<View />

Instead of showing the name for each country, I want to show the country's flag. Luckily, there's another JSON with exactly that, the emoji flag for each country.

So, we fetch that JSON and add the country's flag to each row.

<View />

Then we change a few things in the chart:
...
It would be nice to be able to click on different countries to go to a details page.

So I wrap the map nodes inside `Link` components.
